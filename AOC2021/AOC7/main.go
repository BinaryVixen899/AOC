...
func fuelcalculation(inputslice []int, mode int) {
	var fueltotal int
	var addfuel int
...
func adjustedfuelcalculation(inputslice []int, mode int) {
	var fueltotal int
	var addfuel int
	...
	for _, v := range inputslice {
		...
		// print(addfuel)
		addfuel = costcalculator(addfuel)
		fueltotal += addfuel
...
func costcalculator(fuelcost int) int {
	var cost int
	// start this at 1
	for i := 1; i <= fuelcost; i++ {
		cost = cost + (1 * i)
	}
...
func mode(aslice []int) int {
	...
	if n%(len(aslice)) == 0 {
		...
		remainder = n % len(aslice)
		// sum = n
		// order matters here
		n = n / len(aslice)
		n = round(remainder, len(aslice), n)
	}
	return n
}
