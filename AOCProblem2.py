...
# myArray = 
# [3093,749,3469,142,2049,3637,1596,3035,2424,3982,3290,125,249,131,118,3138],
...
# print(max(myArray[0]) - min(myArray[0]))
# print(len(myArray))

# for row in myArray:
#     print(row)
#     a = max(myArray[onlist])
#     b = min(myArray[onlist])
#     rowtotal = (a-b) + rowtotal
#     print('rowtotal is',rowtotal)
#     onlist = onlist + 1

# print(max(myArray) - min(myArray ))
...
# about the evenly divisible values in the spreadsheet. Unfortunately, none of us are equipped for that kind of calculation - 
...
for row in myArray:
    rowlength = len(row)
    #perform on each digit in row
    for digit in row:
        testdigit = digit
        print('testdigit', digit)
        digittobedivided = currentdigitposition + 1
        print(row[currentdigit])
        print(row[digittobedivided])  
        
        if row[digittobedivided] % row[currentdigit] != 0:
            #okay so like note for later when you're less tired. use the if else statement butalso use like != 0 get it? so that way it does not like yeah you get the point
            #sorry past alexandria but that's wrong, it's' actually because we didn't use row[currentdigit]
            currentdigitposition = currentdigitposition + 1
            
        else:
            rowtotal = (row[digittobedivided] // row[currentdigit] ) + rowtotal
            currentdigitposition = currentdigitposition + 1
            
        print(currentdigitposition)
        print("rowtotal is",rowtotal)
    print(rowtotal)
