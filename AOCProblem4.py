...
passphrasecollection = passphrases.splitlines()
print("passphrasecollection",passphrasecollection)

currentitemstep = 0 
invalidcount = 0 
# #because let's go for gold here, file stuff <---- not at the moment
# with open('passphrases.txt', 'r') as f:
# myNames = [line.strip() for line in f]

#deciphering
#passphraselist = list(passphrases.split(" "))


for passphrase in passphrasecollection:
    currentitemstep = 0
    passphrase = passphrase.split()
    print("current passphrase",passphrase)
    for phrase in passphrase: 
        currentitem = phrase

        print("current phrase",phrase)
    
        for phrase in passphrase[currentitemstep + 1:]:
            if phrase == currentitem:
                print ("Current passphrase is", passphrase)
                print("INVALID")
                invalidcount+= 1
                fullbreak = True
                break
